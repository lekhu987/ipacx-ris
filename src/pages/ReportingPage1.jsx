...
const handleAddendum = (originalReport) => {
  ...
  const displayPatientName =
    r.addendum_index && r.addendum_index > 0
      ? `${r.patient_name} PN(${r.addendum_index})`
      : r.patient_name;
  ...
  return {
    ...
    patient_name: displayPatientName, // 👈 IMPORTANT
 ...
 useEffect(() => {
  ...
  // ========================== Handlers ==========================
 const openReportPanel = (studyUid) => {
  if (!studyUid) return;

  navigate(`/report-panel?study=${encodeURIComponent(studyUid)}`);
  setShowTable(false);
};
